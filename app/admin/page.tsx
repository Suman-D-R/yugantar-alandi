  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const response = await fetch(
        `/api/admin/forms?page=${page + 1}&limit=${pageSize}&search=${search}&sortField=${sortField}&sortOrder=${sortOrder}`
      );
    ...
    <Box sx={{ height: 600, width: "100%" }}>
      {loading ? (
        ...
        <DataGrid
          ...
          filterMode="server"
          onFilterModelChange={(filterModel) => setSearch(filterModel.quickFilterValues?.[0] || "")}
        />
      )}
    </Box>
  );
